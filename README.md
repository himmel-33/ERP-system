...
| Business | 외주 | 협력업체 등록원 Master-Detail, 심사상태, 발주공종, 첨부파일 | `/erp/subcontract/vendor-registrations` |
...
외주 메뉴는 협력업체 등록 명세를 반영한 전용 Master-Detail 화면으로 연결됩니다.

- 거래처, 등록일, 심사진행상태, 최종심사 여부로 조회하며 등록일 종료일(To)을 필수로 검증합니다.
- 조회 조건이 바뀌면 기존 결과를 비우고 `재조회 필요` 상태를 표시해 오래된 결과의 오사용을 막습니다.
- `1010(작성중)` 상태이고 현재 사용자에게 저장 권한이 있을 때만 일반정보, 발주공종, 첨부파일을 변경할 수 있습니다.
- 사업자번호, 등록일, 등록순번의 복합 업무 키 중복을 검사합니다.
- 발주공종 찾기에서 공종을 선택하면 대공종이 자동 연결되고, 공종 변경사항은 상세 저장 전까지 임시 상태로 유지됩니다.
- 첨부파일은 확장자와 10MB 크기 제한을 검사하고 파일명을 정규화합니다. 조회 전용 상태에서는 보기만 제공합니다.

현재 협력업체 등록원은 화면 정책을 검증하기 위한 프런트엔드 프로토타입입니다. 데이터는 브라우저 메모리에 유지되며, Oracle 연동 단계에서는 등록원 업무 키와 내부 ID를 함께 사용하는 REST API로 교체할 수 있도록 화면 상태를 분리했습니다.
